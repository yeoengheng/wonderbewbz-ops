...
export const customerQueries = {
  ...
  async create(customer: Omit<Customer, "customer_id" | "created_at" | "updated_at">) {
    const { data, error } = await (supabase as any).from("customers").insert(customer).select().single();
...
export const bagQueries = {
  ...
  async create(bag: Omit<IndividualBag, "bag_id" | "created_at" | "updated_at">) {
    const { data, error } = await (supabase as any).from("individual_bags").insert(bag).select().single();
  ...
  async createBatch(bags: Omit<IndividualBag, "bag_id" | "created_at" | "updated_at">[]) {
    const { data, error } = await (supabase as any).from("individual_bags").insert(bags).select();
    ...
    return {
      ...
      ordersByStatus:
        ordersResult.data?.reduce(
          (acc, row) => {
            acc[(row as any).status] = (acc[(row as any).status] ?? 0) + 1;
            return acc;
          },
          {} as Record<string, number>,
        ) ?? {},
      machineRunsByStatus:
        machineRunsResult.data?.reduce(
          (acc, row) => {
            acc[(row as any).status] = (acc[(row as any).status] ?? 0) + 1;
            return acc;
          },
